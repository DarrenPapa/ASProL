	...
	def run(self,code):
		...
		while self.p < len(prog):
			...
			## Branching / jumps
				...
				self.p = args[0]-1
				continue
			elif ins == 'riz' and argc == 1: # run the code inside if the value is zero
				if args[0] != 0:
					while self.p < len(prog):
						...
						self.err('[Error]: Branch wasnt closed with `end`!')
			elif ins == 'rnz' and argc == 1: # run the code inside if the value is not zero
				if args[0] != 0:
					while self.p < len(prog):
						line = prog[self.p][0] if len(prog[self.p]) > 0 else ''
						if line == 'end':
			...
			## Memory
				...
				self._memory = [0]*32000
			elif ins == "purge_all" and argc == 0:
				self.__init__()
			## Miscalleneous
			elif ins == 'hlt' and argc == 0:
				sys.exit()
			elif ins == 'check_file' and argc == 2:
				self.write_mem(args[1],1) if os.path.isfile(args[0]) else self.write_mem(args[1],0)
			elif ins.startswith(';['):
				while self.p < len(prog):
					line = prog[self.p][0] if len(prog[self.p]) > 0 else ''
					if line == '];':
